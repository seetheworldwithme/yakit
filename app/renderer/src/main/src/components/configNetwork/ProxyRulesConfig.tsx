...
const ProxyRulesConfig = (props: ProxyRulesConfigProps) => {
              ...
              <div style={{ display: 'flex', flexDirection: 'column', maxHeight: 100, overflow: 'auto' }}>
                {EndpointIds.map((id) => {
                  ...
                  return Url ? (
                    <div key={id}>
                      {Url}{' '}
                      {Disabled ? <span className={styles['disabled-badge']}>{t('ProxyConfig.disabled')}</span> : null}
                    </div>
                  ) : null
                })}
              </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div>
            {!isEndpoints && (
              ...
              >
                <YakitButton type="outline1" colors="danger" disabled={!(isEndpoints ? Endpoints : Routes).length}>
        ...
        >
          {isEndpoints ? (
            ...
            <>
              ...
              >
                <YakitSelect mode="tags" placeholder={t('ProxyConfig.destination_address_placeholder')} />
              </Form.Item>
  ...
  if (visible === undefined) {
    return (
      <div className={styles['proxy-rules-config-overlay']} style={{ height: '100%' }}>
        {drawerContent}
      </div>
    )
  }
...
export const ProxyTest = memo((props: ProxyTestProps) => {
