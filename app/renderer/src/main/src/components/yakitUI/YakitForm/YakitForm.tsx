  ...
  const renderContentValue = useMemoizedFn(() => {
    switch (renderType) {
      case 'textarea':
        return (
          <YakitInput.TextArea
            placeholder={multiple ? t('YakitFormDragger.pathManualInput') : t('YakitFormDragger.pathManualEntry')}
            value={fileName || name}
            disabled={disabled}
            {...textareaProps}
            onChange={(e) => {
...
export const YakitFormDraggerContentPath: React.FC<YakitFormDraggerContentPathProps> = React.memo((props) => {
  ...
  return (
    ...
    >
      <YakitDraggerContentPath {...restProps} size={size} textAreaType={textAreaType} onTextAreaType={onTextAreaType} />
    </Form.Item>
  )
})
