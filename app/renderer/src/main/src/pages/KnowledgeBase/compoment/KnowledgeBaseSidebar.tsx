    ...
    try {
      if (all) {
        setInstallOnlineRagsTokens((prev) => {
          const newTokens = onlineRagList.map((item) => item.installToken).filter((token) => !prev.includes(token))
            ...
            <div className={styles['knowledge-base-info-context']}>
              <div className={styles['knowledge-base-info-header']}>
                ...
                <div className={styles['header-operate']}>
                  <AddKnowledgenBaseDropdownMenu setKnowledgeBaseID={setKnowledgeBaseID} setAddMode={setAddMode} />
              ...
              <div className={styles['repository-manage-search']}>
                <YakitInput
                  ...
                  onPressEnter={() =>
                    setKnowledgeBase(() =>
                      knowledgeBases.filter((it) =>
                        it?.KnowledgeBaseName?.toLowerCase().startsWith(sidebarSearchValue.toLowerCase()),
                      ),
                    )
                  }
                />
                    ...
                    return (
                      <div>
                        ...
                        {!allLatest && (
                          ...
                          >
                            {allDownloadToken ? `下载中... (${allDownloadProgress}%)` : '一键下载'}
                          </YakitButton>
                        )}
                      </div>
                    )
                      ...
                      {onlineRagList?.length > 0 ? (
                        (() => {
                          ...
                          return onlineRagList?.map((items) => {
                            ...
                            // 是否已是最新（版本 hash 对齐）
                            const isLatest = isDownloaded && local.SerialVersionID === items.hash

                            // 是否正在下载
                            const downloadingProgress = onlineRagProgress?.[items.installToken]
                            const isDownloading = downloadingProgress !== undefined && downloadingProgress < 100
                                ...
                                <div className={styles['content']}>
                                  <div className={classNames([styles['header']])}>
                                    <div className={styles['title-left']}>
                                      <div className={styles['title']}>{items.name_zh?.trim() || '-'}</div>
                                    ...
                                    {/* 右侧状态区 */}
                                    {isDownloading ? (
                                      <div className={styles['downloading']}>正在下载..（{downloadingProgress}%）</div>
                                    ) : !isDownloaded ? (
                                      (() => {
                                        const isLoading = installOnlineRagsTokens.includes(items.installToken)
                                        return (
                                          <YakitButton
                                            type="outline1"
                                            loading={isLoading}
                                            disabled={isLoading}
                                            onClick={() => onDownloadOnlineRag(items)}
                                          >
                                            下载
                                          </YakitButton>
                                        )
                                  ...
                                  <div className={styles['description']}>{items.version || '-'}</div>
                                </div>
                              </div>
                            )
                          })
