...
const AIToolEditor: React.FC<AIToolEditorProps> = React.memo((props) => {
  ...
  /** 获取编辑页面的打开的来源 */
  const getModifyAIToolSource = useMemoizedFn(() => {
    const currentItem: PageNodeItemProps | undefined = queryPagesDataById(
      YakitRoute.ModifyAITool,
      YakitRoute.ModifyAITool,
    )
    return currentItem?.pageParamsInfo.modifyAIToolPageInfo?.source
  })
  /** 获取新建页面的打开的来源 */
  const getAddAIToolSource = useMemoizedFn(() => {
    const currentItem: PageNodeItemProps | undefined = queryPagesDataById(YakitRoute.AddAITool, YakitRoute.AddAITool)
    return currentItem?.pageParamsInfo.addAIToolPageInfo?.source
  })
