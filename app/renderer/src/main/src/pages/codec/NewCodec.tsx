        ...
        label: (
          <div className={classNames(styles['extra-menu'], styles['menu-menu-size'])}>
            ...
            <div className={styles['menu-name']}>
              {noInputWordwrap ? t('NewCodecRightEditorBox.autoWordwrap') : t('NewCodecRightEditorBox.noAutoWordwrap')}
            </div>
          </div>
        ),
      },
      ...
      },
      {
        ...
        label: (
          <div className={classNames(styles['extra-menu'], styles['menu-menu-size'])}>
            <EnterOutlined />
            <div className={styles['menu-name']}>
              {noOutputWordwrap ? t('NewCodecRightEditorBox.autoWordwrap') : t('NewCodecRightEditorBox.noAutoWordwrap')}
            </div>
          </div>
        ),
      },
          ...
          <div className={classNames(styles['input-box'], styles['editor-box'])}>
            <div className={styles['header']}>
              <div className={styles['title']}>
                ...
                {inputObj.hidden && (
                  <Tooltip title={size && size.width > 450 ? null : t('NewCodecRightEditorBox.largeInput')}>
                    <YakitTag style={{ marginLeft: 8 }} color="danger">
                      <div className={styles['tag-box']}>
                        <ExclamationIcon className={styles['icon-style']} />
                        {size && size.width > 450 && <span>{t('NewCodecRightEditorBox.largeInput')}</span>}
                      </div>
                    </YakitTag>
                  </Tooltip>
                )}
            ...
            <YakitSpin spinning={runLoading}>
              <div className={styles['header']}>
                <div className={styles['title']}>
                  ...
                  {outPutObj.hidden && (
                    <Tooltip title={size && size.width > 450 ? null : t('NewCodecRightEditorBox.largeOutputTip')}>
                      <YakitTag style={{ marginLeft: 8 }} color="danger">
                        <div className={styles['tag-box']}>
                          <ExclamationIcon className={styles['icon-style']} />
                          {size && size.width > 450 && <span>{t('NewCodecRightEditorBox.largeOutputTip')}</span>}
                        </div>
                      </YakitTag>
                    </Tooltip>
                  )}
...
export const NewCodecMiddleTypeItem: React.FC<NewCodecMiddleTypeItemProps> = (props) => {
  const { t } = useI18nNamespaces(['codec', 'yakitUi'])
    ...
    >
      ...
      {node?.map((item, index) => {
        switch (item.type) {
          ...
          case 'checkbox':
            return (
              <div key={`${data.key}-${index}`} style={{ display: 'inline-block', marginTop: 8, marginRight: 8 }}>
                {onShowUI(item, index)}
              </div>
            )
          default:
...
export const CodecRunListHistoryStore: React.FC<CodecRunListHistoryStoreProps> = React.memo((props) => {
  ...
  return (
    <div className={styles['codec-run-list-history-store']}>
      <div className={styles['header']}>
        <div className={styles['title']}>{t('CodecRunListHistoryStore.historyStore')}</div>
        {mitmSaveData.length !== 0 && (
          <YakitPopconfirm
            title={t('CodecRunListHistoryStore.clearHistoryConfirm')}
            onConfirm={() => {
              removeItem('', true)
            }}
            placement="right"
    ...
    <div className={styles['new-codec-run-list']}>
        ...
        <Droppable droppableId="right" direction="vertical">
          {(provided) => (
            <div
              ...
              style={
                rightItems.length > 0 ? { height: '100%', overflow: 'auto' } : { height: '100%', overflow: 'hidden' }
              }
            >
              {rightItems.length > 0 ? (
                <>
                  {rightItems.map((item, index) => (
                    <Draggable key={`run-${item.key}`} draggableId={`run-${item.key}`} index={index}>
                      {(provided, snapshot) => (
                        <div
                          ref={provided.innerRef}
                          {...provided.draggableProps}
                          style={{
                            ...getMiddleItemStyle(snapshot.isDragging, provided.draggableProps.style),
                          }}
                        >
                ...
                <div className={styles['no-data']}>
                  <YakitEmpty title={t('NewCodecMiddleRunList.dragHint')} />
                </div>
              )}
      ...
      >
        <div className={styles['codec-save-modal']}>
          <YakitInput.TextArea
            value={filterName}
            placeholder={t('NewCodecMiddleRunList.codecOrderNamePlaceholder')}
            showCount
            maxLength={50}
            onChange={(e) => {
