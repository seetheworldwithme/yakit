...
const InstructionsWithSearch: React.FC = React.memo(() => {
  ...
  const highlightText = useMemoizedFn((keyword: string) => {
    ...
    contentRef.current.innerHTML = originalHtmlRef.current.replace(
      regex,
      '<mark data-search-highlight="true">$1</mark>',
    )
...
export const MatcherAndExtraction: React.FC<MatcherAndExtractionProps> = React.memo(
  React.forwardRef((props, ref) => {
    ...
    const onAddMatcher = useMemoizedFn(() => {
      if (
        matcher.matchersList.filter((i) => i.SubMatchers.findIndex((ele) => isMatcherItemEmpty(ele)) !== -1).length > 0
      ) {
              ...
              <div className={styles['matching-extraction-condition']}>
                <div className={styles['matching-extraction-condition-left']}>
                  <div className={styles['condition-mode']}>
                    <span className={styles['condition-mode-text']}>{t('MatcherCollapse.filter_mode')}</span>
                    <YakitRadioButtons
                ...
                {item.SubMatchers.map((matcherItem, subIndex) => (
                  <YakitPanel
                    header={
                      <div className={styles['collapse-panel-header']}>
                        ...
                        <span>[{matcherTypeList(t).find((e) => e.value === matcherItem.MatcherType)?.label}]</span>
                  ...
                  >
                    <MatcherItem
                      ...
                      onEdit={(field, value) => onEditSubMatcher({ field, value, subIndex, index: number })}
                      httpResponse={httpResponse}
                      isSmallMode={isSmallMode}
                    />
                  </YakitPanel>
        ...
        <LabelNodeItem label={t('MatcherItem.match_range')} column={isSmallMode}>
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            <YakitSegmented
              ...
              options={[t('HTTPFuzzerPageTable.response'), t('HTTPFuzzerPageTable.request')]}
            />
