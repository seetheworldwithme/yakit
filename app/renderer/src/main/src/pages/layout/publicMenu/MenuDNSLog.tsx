      ...
      <div className={styles['list-info-wrapper']}>
        <div className={styles['list-header-wrapper']}>
          ...
          <div className={styles['extra-header-body']}>
            {t('Layout.MenuDNSLog.showOnlyARecords')}
            <YakitSwitch
        ...
        <div className={styles['list-body']}>
          <div className={styles['body-header']}>
            <div className={classNames(styles['opt-style'], styles['opt-type'])}>{t('Layout.MenuDNSLog.type')}</div>
            <div className={classNames(styles['opt-style'], styles['opt-ip'])}>{t('Layout.MenuDNSLog.remoteIP')}</div>
            <div className={classNames(styles['opt-style'], styles['opt-time'])}>{t('Layout.MenuDNSLog.time')}</div>
            <div className={classNames(styles['opt-style'], styles['opt-btn'])}>{t('YakitTable.action')}</div>
          </div>
              ...
              {records.map((item) => {
                return (
                  <div key={`${item.RemoteAddr}-${item.Timestamp}`} className={styles['list-opt']}>
                    <div className={classNames(styles['opt-style'], styles['opt-type'])}>{item.DNSType}</div>
      ...
      <div className={styles['dnslog-generate-host']}>
        <div className={!!domain ? styles['generated-wrapper'] : styles['generate-wrapper']}>
          ...
          {!!domain ? (
            ...
            <YakitButton key={'create'} size="small" loading={tokenLoading} onClick={update}>
              {t('Layout.MenuDNSLog.generateDomain')}
            </YakitButton>
          )}
        </div>
        {!!domain && (
        ...
        {!!domain && isHide && records.length > 0 && (
          <YakitButton type="text" onClick={onInfoAll}>
            {t('Layout.MenuDNSLog.viewAccessRecords')}
          </YakitButton>
        )}
      </div>
        ...
        <div className={styles['dnslog-arrow-right-body']}>
          <div className={styles['title-style']}>
            {!!domain ? (
              ...
              <>{t('Layout.MenuDNSLog.accessRecords')}</>
            )}
          </div>
      ...
      <div className={styles['dnslog-list-wrapper']}>
        <div className={styles['dnslog-list-body']}>
          {lastRecords.map((item, index) => {
            return (
              <div key={`${item.RemoteAddr}-${item.Timestamp}-${index}`} className={styles['list-opt-wrapper']}>
                <div className={classNames(styles['opt-style'], styles['opt-type'])}>{item.DNSType}</div>
                <div className={classNames(styles['opt-style'], styles['opt-ip'])}>{item.RemoteIP}</div>
                <div className={classNames(styles['opt-style'], styles['opt-time'])}>{formatTime(item.Timestamp)}</div>
                <div className={classNames(styles['opt-style'], styles['opt-btn'])} onClick={() => onInfoDetails(item)}>
                  {t('YakitButton.detail')}
                </div>
              </div>
            )
