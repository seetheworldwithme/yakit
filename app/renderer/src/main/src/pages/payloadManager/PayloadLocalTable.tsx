...
export const NewPayloadTable: React.FC<NewPayloadTableProps> = (props) => {
      ...
      title: () => (
        <div className={styles['order']}>
          {(response?.Data || []).length > 0 && (
            <YakitCheckbox
              indeterminate={selectPayloadArr.length > 0 && selectPayloadArr.length !== response?.Data.length}
            ...
            <YakitButton
              ...
              onClick={() => {
                const m = showYakitModal({
                  ...
                  maskClosable: false,
                  content: (
                    <PayloadAddEditForm record={record} onUpdatePayload={onUpdatePayload} onClose={() => m.destroy()} />
                  ),
                })
              }}
            >
  ...
  const isEditing = (record: Payload, dataIndex) => record.Id === editingObj?.Id && dataIndex === editingObj?.dataIndex
    ...
    <div className={styles['new-payload-table']}>
      <Table
        ...
        pagination={{
          ...
          showTotal: (i) => <span className={styles['show-total']}>{t('NewPayloadTable.recordCount', { i })}</span>,
