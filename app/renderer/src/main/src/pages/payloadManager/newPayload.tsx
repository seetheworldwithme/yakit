...
export const NewPayloadLocalList: React.FC<NewPayloadLocalListProps> = (props) => {
  ...
  const onUpdateAllPayloadGroup = useMemoizedFn((data: DataItem[]) => {
    ...
    if (!isEqual) {
      ipcRenderer
        ...
        .catch((e: any) => {
          failed(t('NewPayloadLocalList.updateFailed', { error: e }))
        })
    }
  })
  ...
  const getOnlyFolderName = useMemoizedFn(() => {
    ...
    let index = 1
    do {
      newFolderName = `${t('NewPayloadLocalList.unnamedDetection')}${index}`
      index++
  ...
  const onDragEnd = useMemoizedFn((result: DropResult, provided: ResponderProvided) => {
    try {
      ...
      // 跨层级拖拽 删除原有的 新增新添的
        ...
        // 由内部拖向另一个内部
        else if (source.droppableId !== 'droppable-payload' && destination.droppableId !== 'droppable-payload') {
  ...
  return (
    <>
        ...
        <div className={styles['header']}>
          <div className={styles['title-box']}>
            <div className={styles['title']}>
              {onlyInsert
                ? t('NewPayloadLocalList.selectDictionaryToInsert')
          ...
          <div className={styles['content']}>
            <div className={styles['drag-list']}>
              {onlyInsert ? (
                <>
                  {data.map((item, index) => {
                    const fileOutside = moveLevel?.draggableId === item.id ? moveLevel.level === 'outside' : true
                ...
                <>
                  <div className={styles['checkbox-wrapper']}>
                    <YakitCheckbox
                      ...
                      indeterminate={exportData.length > 0 && exportData.length < allCheckIds.length}
      ...
      {exportVisible && (
        <ExportByPayloadGrpc group={exportData.join(',')} setExportVisible={setExportVisible} exportType={exportType} />
      )}
    </>
  )
}
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <div
            ...
            className={classNames(styles['folder'], {
              ...
              [styles['folder-border']]: !isCombine && !menuOpen && !isDragging && !expandArr.includes(folder.id),
            })}
            onClick={() => {
              if (onlyInsert) {
          ...
          >
            <div className={styles['folder-header']}>
              {!onlyInsert && (
                <YakitCheckbox
                  checked={allCheckIds.length > 0 && allCheckIds.every((element) => exportData.includes(element))}
                  disabled={allCheckIds.length === 0}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setExportData && setExportData((old) => filterItem([...old, ...allCheckIds]))
                    } else {
                      setExportData &&
                        setExportData((old: string[]) => old.filter((item) => !allCheckIds.includes(item)))
                    }
                  }}
          ...
          {expandArr.includes(folder.id) && (
            <Droppable
              ...
              renderClone={(provided, snapshot, rubric) => {
                const file: DataItem[] = folder.node?.filter((item, index) => `${item.id}` === rubric.draggableId) || []
                ...
                return (
                  ...
                  >
                    <>
                      {file.length > 0 && (
                        <FileComponentClone file={file[0]} selectItem={selectItem} isInside={fileInside} />
                      )}
                    </>
                  </div>
                )
                ...
                <>
                  <YakitCheckbox
                    className={classNames(styles['payload-checkbox'], {
                      [styles['payload-dark-checkbox']]: file.id !== selectItem && theme === 'dark',
                    })}
...
export const PayloadLocalContent: React.FC<PayloadLocalContentProps> = (props) => {
  ...
  const onQueryEditor = useMemoizedFn((Group: string, Folder: string) => {
    ...
    ipcRenderer
      ...
      .catch((e: any) => {
        failed(t('PayloadLocalContent.editorDataGetFailed'))
      })
      .finally(() => {
        setLoading(false)
      })
  ...
  const onCopyOrMoveFun = useMemoizedFn((id?: number, isCopy = false) => {
    return new Promise((resolve, reject) => {
      if (copyMoveValueRef.current === undefined) {
        warn(t('PayloadLocalContent.pleaseSelectDictionary'))
        resolve(false)
      } else {
        ...
        ipcRenderer
          ...
          .catch((e: any) => {
            failed(t('PayloadLocalContent.operationFailed', { error: e }))
            resolve(false)
          })
          .finally()
      }
  ...
  return (
    <div className={styles['payload-content']}>
            ...
            <YakitButton
              onClick={() => {
                const m = showYakitModal({
                  ...
                  content: (
                    <CreateDictionaries
                      title={t('PayloadLocalContent.extendTo', { name: group })}
                      type="payload"
                      onQueryGroup={() => {
                        emiter.emit('refreshListEvent')
                      }}
      ...
      >
        <UploadStatusInfo
          title={t('PayloadLocalContent.autoRemoveDuplicateWaiting')}
          streamData={streamData}
          cancelRun={() => {
            cancelRemoveDuplicate()
            setVisible(false)
...
export const ExportByPayloadGrpc: React.FC<ExportByPayloadGrpcProps> = (props) => {
  ...
  return (
    ...
    >
      <UploadStatusInfo
        title={t('ExportByPayloadGrpc.exportWaiting')}
...
export const NewPayload: React.FC<NewPayloadProps> = (props) => {
  ...
  const onQueryGroup = (obj?: { Group: string; Folder: string }) => {
    ...
    ipcRenderer
      ...
      .catch((e: any) => {
        failed(t('NewPayload.getDictionaryListFailed', { error: e }))
      })
      .finally(() => {
        setTimeout(() => {
    ...
    <div className={styles['new-payload']} id="new-payload">
      ...
      {data.length === 0 ? (
        <div className={styles['no-data']}>
          <YakitEmpty
            title={t('NewPayload.noPayloadDictionary')}
              ...
              <YakitButton
                ...
                onClick={() => {
                  const m = showYakitModal({
                    ...
                    content: (
                      <CreateDictionaries
                        title={t('NewPayload.newDictionary')}
                        type="dictionaries"
                        onQueryGroup={onQueryGroup}
                        onClose={() => {
                          setExpand(true)
        ...
        <>
          {showContentType ? (
            ...
            <div className={styles['no-data']}>
              <YakitEmpty title={t('NewPayload.selectDictionaryHint')} />
            </div>
          )}
        </>
      )}
      <YakitHint
        ...
        title={t('NewPayload.migrateData')}
        content={t('NewPayload.migrateDataContent')}
        footer={
      ...
      <YakitHint
        ...
        title={t('NewPayload.engineVersionTooLow')}
        content={t('NewPayload.engineVersionTooLowContent')}
        footer={
          <div style={{ marginTop: 24, textAlign: 'right' }}>
            <YakitButton
              size="max"
      ...
      >
        <UploadStatusInfo
          title={t('NewPayload.migratingDataWaiting')}
          streamData={streamData}
          cancelRun={() => {
