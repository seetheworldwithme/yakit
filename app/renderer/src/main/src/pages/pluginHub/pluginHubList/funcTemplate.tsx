  ...
  return (
    ...
    >
      ...
      <div className={styles['hub-list-filter-body']}>
        ...
        {groupList.length === 0 && (
          <YakitEmpty style={{ paddingTop: 48 }} title={noDataHint || t('YakitEmpty.noData')} />
        )}
      </div>
    </div>
  )
    ...
    <div ref={listRef} className={styles['hub-grid-list']} onScroll={() => onScrollCapture()}>
      <div ref={containerRef}>
        {list.map((item, index) => {
          ...
          return (
            <div key={`${arr.map((info) => info[keyName]).join('-')}`} className={styles['hub-grid-list-row']}>
              {arr.map((el, i) => {
                const order = item.index * gridCol + i
                return (
                  <div
                    key={el[keyName]}
                    className={classNames(styles['hub-grid-list-opt'], styles[`grid-opt-${gridCol}`])}
                  >
    ...
    >
      <div className={styles['plugin-details-item']}>
        <div className={styles['plugin-details-item-info']}>
          ...
          {authorImgNode}
          <div
            className={classNames(styles['plugin-details-item-info-text-style'], 'yakit-content-single-ellipsis')}
            title={pluginName}
          >
            {pluginName}
          </div>
