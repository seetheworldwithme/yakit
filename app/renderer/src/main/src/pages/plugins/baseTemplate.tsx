      ...
      <div className={styles['header-wrapper']}>
        <div className={styles['header-info']}>
          <div className={styles['info-title']}>
            <div className={classNames(styles['title-style'], 'yakit-content-single-ellipsis')} title={pluginName}>
              {pluginName || '-'}
            </div>
          ...
          <div style={{ minWidth: tagMinWidth || 20 }} className={classNames(styles['info-tags'])}>
            {pluginTypeToName[type] && pluginTypeToName[type].name && (
              <YakitTag color={pluginTypeToName[type]?.color as any}>{pluginTypeToName[type]?.name}</YakitTag>
            )}
...
export const PluginModifyInfo: React.FC<PluginModifyInfoProps> = memo(
  React.forwardRef((props, ref) => {
    ...
    return (
      <Form className={classNames('plugins-form', styles['plugin-modify-info-wrapper'])} form={form} layout="vertical">
        <Form.Item
          label={
            <>
                ...
                <div key={item.CVE} className={styles['risk-info-opt']}>
                  ...
                  <div className={styles['bug-info']}>
                    <div className={styles['info-title']}>
                      ...
                      <YakitTag color={RiskLevelToTag[item?.Level || 'info'].color as YakitTagColor}>
                        {RiskLevelToTag[item?.Level || 'info'].name}
                      </YakitTag>
                    </div>
                    <div className={styles['info-content']}>
                      <div className={styles['header-style']}>漏洞描述</div>
                      <div className={styles['content-style']}>{item?.Description || '暂无描述'}</div>
                    </div>
                    <div className={styles['info-content']}>
                      <div className={styles['header-style']}>修复建议</div>
                      <div className={styles['content-style']}>{item?.Solution || '暂无修复建议'}</div>
                    </div>
                  </div>
                </div>
              )
              ...
              {data.EnablePluginSelector && (
                <div className={classNames(styles['switch-wrapper'], styles['switch-type-wrapper'])}>
                  ...
                  <div className={styles['select-wrapper']}>
                    <YakitSelect
                      size="large"
                      value={data.PluginSelectorTypes ? data.PluginSelectorTypes.split(',') : undefined}
                      mode="multiple"
                      allowClear
                    ...
                    {selectStatus === 'error' && <span className={styles['switch-text-error']}>联动插件类型必填</span>}
                  </div>
                </div>
              )}
            </div>
                ...
                <YakitSwitch
                  ...
                  onChange={(check) => {
                    if (check) {
                      ...
                      const arr = tags.filter((item) => item !== PluginSwitchToTag.PluginYakDNSLogSwitch)
                      setTags([...arr])
                    }
                  }}
                />
              ...
              <YakitSwitch
                ...
                onChange={(check) => {
                  if (check) {
                    ...
                    const arr = tags.filter((item) => item !== PluginSwitchToTag.PluginCodecHttpSwitch)
                    setTags([...arr])
                  }
                }}
              />
              ...
              <YakitSwitch
                ...
                onChange={(check) => {
                  if (check) {
                    ...
                    const arr = tags.filter((item) => item !== PluginSwitchToTag.PluginCodecContextMenuExecuteSwitch)
                    setTags([...arr])
                  }
                }}
              />
              ...
              <YakitSwitch
                ...
                onChange={(check) => {
                  if (check) {
                    ...
                    const arr = tags.filter((item) => item !== PluginSwitchToTag.PluginCodecSingleHistorySwitch)
                    setTags([...arr])
                  }
                }}
              />
              ...
              <YakitSwitch
                ...
                onChange={(check) => {
                  if (check) {
                    ...
                    const arr = tags.filter((item) => item !== PluginSwitchToTag.PluginCodecMultipleHistorySwitch)
                    setTags([...arr])
                  }
                }}
              />
