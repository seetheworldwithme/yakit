                    ...
                    return (
                      <div
                        ...
                        className={classNames(styles['opt-wrapper'], {
                          [styles['opt-active']]: active.findIndex((ele) => ele.key === item.key) !== -1,
                        })}
                        onClick={() => {
  ...
  return (
    ...
    >
      {nameAndIcon ? (
        ...
        <YakitButton isActive={show} icon={icon} onClick={(e) => e.stopPropagation()} {...(button || {})}></YakitButton>
      )}
    </Dropdown>
  )
})
        ...
        <div className={styles['content-style']}>
          ...
          <div className={styles['title-wrapper']}>
            <div className={styles['title-body']}>
              <div className={classNames(styles['title-style'], 'yakit-content-single-ellipsis')}>{title}</div>
              {subtitle()}
            </div>
    ...
    >
      <div ref={wrapperRef}>
        {list.map((ele) => {
          ...
          return (
            <div key={`${itemArr.map((item) => item[keyName]).join('-')}`} className={styles['row-wrapper']}>
    ...
    <div className={styles['grid-layout-opt-wrapper']} onClick={onclick}>
      ...
      <div className={classNames(styles['opt-body'], { [styles['opt-active-body']]: checked })}>
        <div className={styles['opt-content']}>
          <div className={styles['title-wrapper']}>
            <div className={classNames(styles['title-style'], 'yakit-content-single-ellipsis')} title={title}>
              {title}
            </div>
            {subtitle()}
          </div>
          ...
          <div className={styles['content-wrapper']}>
                ...
                {tagList.map((item) => {
                  return (
                    <div key={`tag-${item}`} className={styles['tag-opt']}>
                      <div className={classNames(styles['text-style'], 'yakit-content-single-ellipsis')}>
            ...
            <div className={styles['user-wrapper']}>
              ...
              <div className={styles['contribute-body']}>
                ...
                {contributes.length > 0 && <div className={styles['more-style']}>{`+${contributes.length}`}</div>}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div ref={wrapperRef} className={classNames(styles['tags-list-show-wrapper'], 'yakit-content-single-ellipsis')}>
      {isShow
        ? tags.map((item, index) => {
            return (
              <div ref={index === 0 ? firstRef : undefined} key={`tag-${item}`} className={styles['tag-wrapper']}>
                {item}
              </div>
            )
          })
                  ...
                  return (
                    <div className={styles['list-opt']} key={item.IdKey}>
                      ...
                      <div className={styles['opt-content']}>
                        ...
                        {errorPosition && <span className={styles['error-position']}> {errorPosition}</span>}
                      </div>
                    </div>
                  )
                })}
