  ...
  return (
    <div className={`reverse-table-wrapper ${isPayload ? 'payload-table-padding' : 'reverse-table-padding'}`}>
      ...
      <YakitResizeBox
              ...
              columns={[
                {
                  ...
                  render: (text) => {
                    ...
                    return (
                      <div className={`tag-wrapper tag-${!label ? 'blue' : DefaultTypeClassName[text]}`}>
                        {!label ? text : label}
                      </div>
                    )
                  },
        ...
        secondNode={
          <YakitEditor readOnly={true} type="http" value={Uint8ArrayToString(selectRow?.raw || new Uint8Array())} />
        }
      ></YakitResizeBox>
      {/* <YakitSelect
                mode='multiple'
