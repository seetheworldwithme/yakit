    ...
    arr.forEach((item) => {
      if (item.key === 'ExternalProjectCode' || item.key === 'ExternalModule') {
        header = header.map((itemIn) => {
          if (itemIn.key === item.key) {
            itemIn.name = item.name || itemIn.name
            itemIn.style = { ...itemIn.style, display: 'flex' }
          }
          return itemIn
        })
      }
...
export const NewProjectAndFolder: React.FC<NewProjectAndFolderProps> = memo((props) => {
  ...
  // 添加
  const handleAdd = (e: React.MouseEvent<HTMLElement, MouseEvent>, field: string, ref: React.MutableRefObject<any>) => {
    e.stopPropagation()
    const v = form.getFieldsValue()
  ...
  const isShowExternalProjectCode = useMemo(() => {
    return judgeProjectConfig(eeSystemConfig).find((item) => item.key === 'ExternalProjectCode')
  }, [eeSystemConfig])

  const isShowExternalModule = useMemo(() => {
    return judgeProjectConfig(eeSystemConfig).find((item) => item.key === 'ExternalModule')
  }, [eeSystemConfig])
  return (
    <YakitModal
      title={headerTitle}
        ...
        {isNew && (
          <>
            <Form.Item
              label={
                <div>
                  {`${isFolder ? t('NewProjectAndFolder.folderName') : t('NewProjectAndFolder.projectName')}`}{' '}
            ...
            {!isFolder && !parentNode && isShowExternalProjectCode && (
              ...
              >
                <YakitInput
                  ...
                  value={info.ExternalProjectCode}
                  onChange={(e) => setInfo({ ...info, ExternalProjectCode: e.target.value })}
                />
              </Form.Item>
            )}
            {!isFolder && !parentNode && isShowExternalModule && (
              <Form.Item
                label={
                  <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              >
                <YakitInput
                  ...
                  value={info.ExternalModule}
                  onChange={(e) => setInfo({ ...info, ExternalModule: e.target.value })}
                />
              </Form.Item>
            )}
            {!isFolder && isIRify() && (
        ...
        {isExport && (
          <>
            ...
            <Form.Item
              label={
                <div>
                  {t('NewProjectAndFolder.password')} <span className={styles['required-style']}>*</span> :
                </div>
              }
        ...
        {isImport && (
          <>
            <Form.Item
              label={
                <div>
                  {t('NewProjectAndFolder.selectProjectFile')} <span className={styles['required-style']}>*</span> :
                </div>
              }
              help={
                <div className={styles['import-form-item-help-wrapper']}>
