            ...
            <Form.Item colon={false} label={' '} style={{ marginBottom: 0 }}>
              <div className={styles['space-engine-form-operate']}>
                {isExecuting ? (
                  ...
                  <YakitButton className={styles['space-engine-form-operate-start']} htmlType="submit" size="large">
...
const SpaceEngineFormContent: React.FC<SpaceEngineFormContentProps> = React.memo((props) => {
    ...
    apiGetSpaceEngineStatus(param).then((value) => {
      switch (value.Status) {
        case 'normal':
          break
        default:
          yakitNotify('error', t('SpaceEngineFormContent.spaceEngineVerifyFailed') + value.Info || value.Status)
  ...
  const onSetGlobalNetworkConfig = useMemoizedFn((type) => {
    const initData: ThirdPartyApplicationConfig = globalNetworkConfig.AppConfigs.find((ele) => ele.Type === type) || {
      Type: type,
    }
