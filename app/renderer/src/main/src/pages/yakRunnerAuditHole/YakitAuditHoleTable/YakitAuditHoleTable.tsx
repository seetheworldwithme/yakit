...
export const batchRefreshMenuData: (t: TFunction) => YakitMenuItemProps[] = (t) => {
  return [
    {
      key: 'noResetRefresh',
...
export const YakitAuditHoleTable: React.FC<YakitAuditHoleTableProps> = React.memo((props) => {
  ...
  const getLabelByValue = (value) => {
    ...
    const option = defaultTags(t).find((option) => option.value === value)
  ...
  return (
    <div className={classNames(styles['yakit-audit-hole-table'], riskWrapperClassName)} ref={tableBoxRef}>
                  ...
                  <div className={styles['table-renderTitle-left']}>
                    ...
                    <div className={styles['virtual-table-heard-right']}>
                      <div className={styles['virtual-table-heard-right-item']}>
                        ...
                        <span className={styles['virtual-table-heard-right-number']}>{tableTotal}</span>
                      </div>
                      <Divider type="vertical" />
                      <div className={styles['virtual-table-heard-right-item']}>
      ...
      <YakitModal
        ...
        footer={
          !exportProgressVisible ? (
            <>
              <YakitButton type="outline2" onClick={() => setExportModalVisible(false)} style={{ marginRight: 8 }}>
                {t('YakitButton.cancel')}
              </YakitButton>
              <YakitButton onClick={onConfirmExport}>{t('YakitButton.confirm')}</YakitButton>
            </>
          ) : (
      ...
      >
        {!exportProgressVisible ? (
          <div className={importExportStyles['import-export-modal']}>
            <div className={importExportStyles['export-hint']}>{t('YakitAuditHoleTable.exportHint')}</div>
      ...
      <YakitModal
        ...
        footer={
          !importProgressVisible ? (
            <YakitButton onClick={onConfirmImport}>{t('YakitButton.import')}</YakitButton>
          ) : (
      ...
      >
        {!importProgressVisible ? (
          <div className={importExportStyles['import-export-modal']}>
            <div className={importExportStyles['import-hint']}>{t('YakitAuditHoleTable.importWarning')}</div>
    ...
    <div className={styles['yakit-ssa-risk-select-tag']}>
      <Form onFinish={onFinish} labelCol={{ span: 4 }} wrapperCol={{ span: 20 }}>
        ...
        <div className={styles['yakit-ssa-risk-select-tag-btns']}>
          ...
          <YakitButton htmlType="submit">{t('YakitButton.confirm')}</YakitButton>
        </div>
      </Form>
    </div>
  )
          ...
          <div className={styles['content-heard-body']}>
            ...
            <div className={styles['content-heard-body-description']}>
              ...
              <span className={styles['description-port']}>
                {t('YakitAuditHoleTable.belongsToProject', { name: info.ProgramName || '-' })}
              </span>
              <Divider type="vertical" style={{ height: 16, margin: '0 8px' }} />
...
export const AuditResultHistory: React.FC<AuditResultHistoryProps> = React.memo((props) => {
  ...
  const onDeleteSSARiskDisposals = useMemoizedFn((id: number) => {
    apiDeleteSSARiskDisposals({ Filter: { ID: [id] } })
      ...
      .catch((e) => {
        yakitNotify('error', t('YakitNotification.deleteFailed', { error: String(e) }))
      })
  })
            ...
            >
              <div className={styles['header-content']}>
                ...
                <div className={styles['author-name']}>
                  {t('YakitAuditHoleTable.disposalStatusValue', { value: getLabelByValue(info.Status) })}
                </div>
