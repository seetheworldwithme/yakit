...
**数据前置要求**：执行本技能前，需先使用 `/xlsx-to-sqlite` 将用户提供的 Excel 数据文件导入 SQLite 数据库（`analysis.db`）。本技能的所有分析均基于 SQL 查询，不再使用 pandas 直接读取 xlsx。

## 执行编排（强制）

当用户提出“调用本技能分析某目录下涉税数据”时，必须按以下顺序执行：

1. **先调用 `/xlsx-to-sqlite`**：将用户提供目录中的 xlsx 文件导入 SQLite（建议输出到本次任务专属目录下的 `analysis.db`）
2. **建立标准分析视图**：从通用导入后的原始表中，自动识别并创建 `seller_invoice`、`buyer_invoice`、`tax_registration`、`business_registration`、`bank_transaction` 五个标准视图（实际缺失的表可跳过）
3. **SQL优先执行分析**：按本技能阶段A-H使用 SQL 完成统计、筛查、关联和研判；仅在 SQLite 无法直接表达的少数环节使用最小 Python 辅助
...
- **默认不落地中间 Python 脚本**：不应默认生成 `scripts/analyze_tax.py`、`scripts/gen_reports.py`、`scripts/import_tax_data.py` 等文件。
- **SQL 查询直接执行**：分析阶段优先直接执行 SQL（聚合、筛查、关联、窗口统计）。
- **Markdown 直接写入目标文件**：逐户报告和汇总报告直接写入 `.md` 文件，不需要先生成 `gen_reports.py`。
- **仅在 SQL 无法表达时使用最小 Python**：例如图遍历、复杂文本相似度、少量格式化；且优先使用一次性内联执行，不新增长期脚本文件。
- **只有用户明确要求时才生成脚本文件**：若用户明确要求“沉淀脚本/复用脚本/产出代码文件”，才创建 `.py`。
...
**标准字段**：登记序号、纳税人识别号、社会信用代码、纳税人名称、纳税人状态、纳税人有效标志、课征主题类型代码、课征主题类型名称、跨区税源登记、登记注册类型、登记日期、主管税务局、批准设立日期、法定代表人固定电话、法定代表人移动电话、财务负责人姓名、财务负责人身份证件号码、财务负责人固定电话、财务负责人移动电话、开业设立日期、生产经营地址行政区划数字代码、注册地址行政区划数字代码、法定代表人姓名、法定代表人身份证号码、生产经营地址、注册地址、国有控股类型代码、办税人姓名、办税人身份证件号码、办税人固定电话、办税人移动电话、行业种类

### 2. 销方数据表

目标企业作为销方开具的增值税发票明细，每条记录为一张发票的一条明细行。用于分析开票行为、销售品名、税率分布、购方特征等。

**标准字段**：序号、购方识别号、购方名称、购方地址电话、购方银行账号、购方税务机关、购方发票流向、销方识别号、销方名称、销方税务机关、销方地址电话、销方银行账号、销方发票流向、开具发票类型、发票类型、发票代码、发票号码、发票状态、货品名称、商品编码、明细序号、税率、规格型号、单位、数量、单价、接收时间、货物金额、货物税额、发票金额、发票税额、价税合计、开票日期、作废标志、作废时间、是否勾选认证、认证时间、发票核选用途、发票标志、异地发票标志、IP、MAC、源端口、主板序列号、传入时间
...
**标准字段**：企业名称、登记状态、统一社会信用代码、法定代表人、成立日期、核准日期、注册资本、实缴资本、电话、更多电话、所属省份、所属城市、所属区县、企业地址、通信地址、邮箱、更多邮箱、登记机关、企业(机构)类型、纳税人资质、纳税人识别号、注册号、组织机构代码、营业期限、国标行业门类、国标行业大类、国标行业中类、国标行业小类、曾用名、英文名、企业规模、经营范围、企业简介

### 5. 银行交易明细表（如有提供）

涉案企业及关联人的银行账户交易流水，用于资金回流分析、交易对手地域分析和虚开资金链追踪。

**标准字段**：交易日期/记账日期、交易时间、摘要/交易摘要、交易金额/发生额、收付标志、余额、对方户名/交易对手名称、对方账号/交易对手账号、对方开户行/交易对手开户行、本方户名/账户名称、本方账号/账号、本方开户行、交易类型、备注/附言
...
1. **优先 SQL，最小 Python**：
   - 数据筛查、聚合、关联、排序、窗口统计优先用 SQL
   - 仅在 SQLite 不擅长的环节（如复杂图搜索、文本相似度、报告拼装）使用少量 Python
2. **禁止回退到“pandas 直接读 xlsx”**：xlsx 读取统一由 `/xlsx-to-sqlite` 完成
3. **统一基于标准视图分析**：分析 SQL 不直接绑死原始表名，而是基于 `seller_invoice` / `buyer_invoice` / `tax_registration` / `business_registration` / `bank_transaction`
4. **数值字段防错**：金额字段默认按 TEXT 存储，计算时统一 `CAST(... AS REAL)`
5. **日期字段防错**：日期字段默认按 TEXT 处理，按 `substr(字段,1,7)` 等方式提取年月
6. **缺字段处理**：缺列则跳过对应规则，并在报告中明确写“因缺少XX字段，该项未分析”
7. **结果可追溯**：报告中的关键结论必须能回溯到具体 SQL 查询结果
8. **中文列名参数绑定规则（关键）**：`?`/`:name` 只可绑定“值”，不可绑定表名/列名。动态标识符必须白名单校验后再加双引号转义。
9. **中文路径与文件名兼容**：统一使用 `pathlib.Path` 处理路径，避免手工 `.encode()` / `.decode()`。
10. **终端编码异常处理**：若日志出现乱码，优先检查运行时 stdout/stderr 编码，不直接判定数据库内容异常。
...
由于 `/xlsx-to-sqlite` 现为通用导入（原样建表），实际数据库中的表名通常是 `文件名__工作表名`。因此本技能必须先做“表识别+标准视图映射”，再执行后续 SQL。
...
3. 用“关键列命中数”选择最匹配表，并创建标准视图（示例）：

```sql
CREATE VIEW seller_invoice AS SELECT * FROM "实际销方表名";
CREATE VIEW buyer_invoice  AS SELECT * FROM "实际购方表名";
```
