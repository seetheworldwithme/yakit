...
const HTTPHistoryAnalysis = React.lazy(() =>
  import('@/pages/hTTPHistoryAnalysis/HTTPHistoryAnalysis').then(({ HTTPHistoryAnalysis }) => ({
    default: HTTPHistoryAnalysis,
  })),
)
const ConcurrencyAllRes = React.lazy(() =>
...
const FuzzerSequence: React.FC<FuzzerSequenceProps> = React.memo((props) => {
  ...
  const [sequenceList, setSequenceList] = useState<SequenceProps[]>(queryFuzzerSequenceCacheDataByGroupId(propsGroupId))
  ...
  // 序列模式的响应数据
  const [sequenceResponseMap, { set: setSequenceResponse, get: getSequenceResponse, reset: resetSequenceResponse }] =
    useMap<string, ResponseProps>()
  // 并发模式的响应数据
  const [
    concurrencyResponseMap,
    { set: setConcurrencyResponse, get: getConcurrencyResponse, reset: resetConcurrencyResponse },
  ] = useMap<string, ResponseProps>()
  // 并发展示组的全部响应
  ...
  const currentList = useMemo(
    () => (isConcurrency ? concurrencyList : sequenceList),
    [isConcurrency, concurrencyList, sequenceList],
  )
  const currentOriginList = useMemo(
    () => (isConcurrency ? originConcurrencyList : originSequenceList),
    [isConcurrency, originConcurrencyList, originSequenceList],
  )
  ...
  const loading = useMemo(
    () => (isConcurrency ? concurrencyLoading : sequenceLoading),
    [isConcurrency, concurrencyLoading, sequenceLoading],
  )
  ...
  const onDragEnd = useMemoizedFn((result: DropResult, provided: ResponderProvided) => {
    ...
    if (result.source.droppableId === 'droppable1' && result.destination.droppableId === 'droppable1') {
      ...
      const newList: SequenceProps[] = reorder(currentList, result.source.index, result.destination.index)
      setCurrentList(newList)
    }
  })
  ...
  const onStartExecution = useMemoizedFn(() => {
    ...
    if (i !== -1) {
      ...
      yakitNotify(
        'error',
        t(isConcurrency ? 'FuzzerConcurrency.execute_after_concurrency' : 'FuzzerSequence.execute_after_sequence'),
      )
      return
    }
    ...
    // 并发模式下点击开始执行时，先展示组内全部响应
    isConcurrency && setShowAllRes(true)
    ...
    if (isConcurrency) {
      ...
      if (ConcurrencyAdvancedConfigValue && !ConcurrencyAdvancedConfigValue.disableAdvancedSet) {
        const { repeatTimes, disableUseConnPool, concurrent, minDelaySeconds, maxDelaySeconds } =
  ...
  /**并发模式全部数据的匹配提交 */
  const onConcurrencyMatchSubmit = useMemoizedFn(() => {
    const ConcurrencyAdvancedConfigValue = pageGroupData?.pageParamsInfo?.ConcurrencyAdvancedConfigValue
    if (!ConcurrencyAdvancedConfigValue) return
    ...
    try {
      ...
      const { successFuzzer } = getAllConcurrencyResponse()
      ...
      // 添加高级配置
      if (repeatTimes || disableUseConnPool || concurrent || minDelaySeconds || maxDelaySeconds) {
        if (ConcurrencyAdvancedConfigValue?.disableAdvancedSet) return
        Object.assign(params, {
          Concurrent: 0,
          Overrides: {
            RepeatTimes: repeatTimes,
  ...
  /**并发和序列单个请求的匹配提交 */
  const onSingleMatchSubmit = useMemoizedFn(() => {
    if (!currentSequenceItem || !currentSelectRequest) return

    const { matchers = [], extractors = [] } = currentSelectRequest.advancedConfigValue || {}
    const currentResponse = responseMap.get(currentSequenceItem.id)
    if (!currentResponse || currentResponse.successFuzzer.length === 0) return
    ...
    try {
      ...
      // 清空当前项的所有缓存数据
      const itemId = currentSequenceItem.id
      successCountRef.current.set(itemId, 0)
      failedCountRef.current.set(itemId, 0)
      successBufferRef.current.set(itemId, [])
      failedBufferRef.current.set(itemId, [])
      countRef.current.set(itemId, 0)
      runtimeIdBufferRef.current.set(itemId, [])
      fuzzerResChartDataBufferRef.current.set(itemId, [])
      ...
      setResponse(itemId, {
        ...
        failedFuzzer: [],
        runtimeIdFuzzer: [],
        fuzzerTableMaxData: fuzzerTableMaxDataRef.current.get(itemId) || DefFuzzerTableMaxData,
        fuzzerResChartData: [],
      })
  ...
  const onSelect = useMemoizedFn(async (val: SequenceProps) => {
    if (!val.pageId) {
      yakitNotify(
        ...
        t(
          isConcurrency
            ? 'FuzzerConcurrency.select_after_configuring_concurrency'
            : 'FuzzerSequence.select_after_configuring_sequence',
        ),
      )
      return
    }
    validateME()
      ...
      .finally(() => {
        ...
        isConcurrency && setShowAllRes(false)
      })
  })
  ...
  /** 获取并发模式下所有响应的数据 */
  const getAllConcurrencyResponse = useMemoizedFn((): ResponseProps => {
    const newList = Array.from(responseMap)
    /** 成功计数 */
    let totalSuccessCount = 0
    /** 失败计数 */
    let totalFailedCount = 0
    /** 成功响应数据 */
    let allSuccessFuzzer: FuzzerResponse[] = []
    /** 失败响应数据 */
    let allFailedFuzzer: FuzzerResponse[] = []
    /** 所有 RuntimeID 用于查看全部流量 */
    ...
    newList.forEach(([id, response]) => {
      if (response) {
        ...
        allSuccessFuzzer = [...allSuccessFuzzer, ...response.successFuzzer]
        allFailedFuzzer = [...allFailedFuzzer, ...response.failedFuzzer]
        allRuntimeIds = [...allRuntimeIds, ...response.runtimeIdFuzzer]
        maxFuzzerTableMaxData = Math.max(maxFuzzerTableMaxData, response.fuzzerTableMaxData || DefFuzzerTableMaxData)
        allFuzzerResChartData = [...allFuzzerResChartData, ...(response.fuzzerResChartData || [])]
      }
    })
    ...
    const params = {
      ...
      runtimeIdFuzzer: [...new Set(allRuntimeIds)],
      fuzzerTableMaxData: maxFuzzerTableMaxData,
      fuzzerResChartData: allFuzzerResChartData,
    }
    return params
  })
  ...
  const onSaveMatcherAndExtractionDrawer = useMemoizedFn(
    (matcher: MatcherValueProps, extractor: ExtractorValueProps, isApply?: boolean) => {
      // 如果是并发模式且显示全部结果,保存到 ConcurrencyAdvancedConfigValue
  ...
  const matcherAndExtractionValue: MatcherAndExtractionValueProps = useCreation(() => {
    ...
    if (isShowAllConcurrency) {
      ...
      if (concurrencyConfig) {
        data.matcher = {
          matchersList: concurrencyConfig.matchers || [],
        }
        data.extractor = {
          extractorList: concurrencyConfig.extractors || [],
        }
      }
      return data
    }
    ...
    return data
  }, [currentSelectRequest, visibleDrawer, showMatcherAndExtraction, triggerME, responseMap.size])
  ...
  const renderHistoryAnalysis = useMemoizedFn(() => {
    ...
    const runtimeId =
      isSingle && currentSequenceItem && currentSelectResponse?.runtimeIdFuzzer
    ...
    return (
      <YakitDrawer
        title={
          isSingle
            ? `${t('HTTPFuzzerPage.trafficAnalysisMode')}-${currentItem?.pageName}`
  ...
  const AdvancedConfigActive = useMemo(
    () =>
      pageGroupData?.pageParamsInfo.ConcurrencyAdvancedConfigValue &&
      !pageGroupData?.pageParamsInfo.ConcurrencyAdvancedConfigValue.disableAdvancedSet,
    [pageGroupData?.pageParamsInfo.ConcurrencyAdvancedConfigValue],
  )
  ...
  return (
    <>
      ...
      >
                  ...
                  return (
                    ...
                    >
                      {currentList.map((sequenceItem, index) => {
                        //并发显示全部响应的时候 不选中单个请求
                        const isSelect = !isConcurrencyShowAllRes && currentSequenceItem?.id === sequenceItem.id
                        return (
                          <Draggable
                            key={sequenceItem.id}
        ...
        <div
          className={classNames(styles['fuzzer-sequence-midden'], {
            [styles['fuzzer-sequence-midden-hidden']]:
              !isShowSetting || !currentSequenceItem || isConcurrencyShowAllRes,
          })}
        >
        ...
        <div
          className={classNames(styles['fuzzer-sequence-content'], {
            [styles['fuzzer-sequence-content-no-padding']]: isConcurrencyShowAllRes,
          })}
        >
          {/* 并发点击执行后先展示组的全部响应 后点击每个请求 展示详情 */}
          {isConcurrencyShowAllRes ? (
            <React.Suspense fallback={<div>Loading...</div>}>
              <ConcurrencyAllRes
                responseInfo={getAllConcurrencyResponse()}
                loading={loading}
                matcherValue={matcherAndExtractionValue.matcher}
                extractorValue={matcherAndExtractionValue.extractor}
                groupPageId={selectGroupId}
                onDebug={onDebug}
                onShowAll={onShowAllHeader}
                inViewport={inViewport === true}
                onMatchSubmit={onConcurrencyMatchSubmit}
                onOpenMatcherDrawer={() => {
                  setVisibleDrawer(true)
            ...
            <>
              <SequenceResponseHeard
                ...
                onMatchSubmit={onSingleMatchSubmit}
                onOpenMatcherDrawer={() => {
                  setVisibleDrawer(true)
                  setActiveType('matchers')
                }}
            ...
            <YakitEmpty
              title={t(
                isConcurrency
                  ? 'FuzzerConcurrency.select_webfuzzer_with_concurrency_tip'
                  : 'FuzzerSequence.select_webfuzzer_with_sequence_tip',
              )}
            />
          )}
        </div>
      ...
      <MatcherAndExtractionDrawer
        ...
        setDefActiveType={setActiveType}
        ...
        defActiveKeyAndOrder={defActiveKeyAndOrder}
        hasApplyBtn={!!getAllConcurrencyResponse().successCount}
      />
            ...
            <div className={styles['fuzzer-sequence-list-item-heard-title']}>
              ...
              <YakitPopover
                ...
                content={
                  ...
                  >
                    <div style={{ color: 'var(--Colors-Use-Neutral-Text-1-Title)' }}>{t('SequenceItem.edit_name')}</div>
                    <YakitInput
                      ...
                      <div className={styles['cog-popover-content']}>
                        ...
                        >
                          <YakitSwitch
                            ...
                            onChange={(checked) => onUpdateItem({ ...item, inheritVariables: checked })}
                          />
                        </LabelNodeItem>
                        <LabelNodeItem
                        ...
                        >
                          <YakitSwitch
                            ...
                            onChange={(checked) => onUpdateItem({ ...item, inheritCookies: checked })}
                          />
                        </LabelNodeItem>
                      </div>
                    }
  ...
  return (
    <div className={styles['sequence-response-heard']}>
      <div className={styles['sequence-response-heard-left']}>
        {isConcurrency ? (
          <span className={styles['sequence-response-heard-left-title']}>{currentSequenceItemPageName}</span>
        ) : (
          <span>
      ...
      <div style={{ display: 'flex' }}>
        {isConcurrency ? (
          <Divider type="vertical" style={{ margin: 8, visibility: 'hidden' }} />
        ) : (
          <>
        ...
        <div className={styles['hot-patch-trigger']}>
          ...
          <HotCodeTemplate
            ...
            onClickHotCode={(temp, tempName) => {
              ...
              emiter.emit('onSelectFuzzerHotPatchTemplate', JSON.stringify({ pageId, templateName: tempName || '' }))
              hotPatchTrigger()
            }}
            triggerNode={
              <YakitButton
                  ...
                  >
                    ...
                    {cachedTotal > 1 ? (
                      ...
                      <Result
                        ...
                        title={t(
                          isConcurrency
                            ? 'ConcurrencyResponse.view_after_executing_concurrency'
                            : 'SequenceResponse.view_after_executing_sequence',
                        )}
                      />
                    )}
                  </div>
